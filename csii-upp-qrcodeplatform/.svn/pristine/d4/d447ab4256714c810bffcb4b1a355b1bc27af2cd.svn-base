...
public class UppExceptionHandler implements InvokerExceptionHandler, ApplicationContextAware {
	...
	public static final String RESPOONSE_EXCEPTION_TAG = "F";
	...
	public void handleException(Exception exception, Context context) {
		...
		if (throwable == null) {
			...
			if (flag) {
				...
				try {
					...
					errType = RESPOONSE_EXCEPTION_TAG;
					errCode = ResponseCode.FAITURE;
		...
		if (!StringUtil.isStringEmpty(blend)) {
			...
			if (index > 0) {
				errType = blend.substring(0, 1);
				errCode = blend.substring(1, index);
