 ...
 *
 */
public class AddPayOnlineTransAction  extends BaseOnlineTransAction {
	...
	protected void prepareOnlineTrans(Context ctx,InputPaymentData inputData) throws PeException {
		...
		List<Map> merSubTransMaps = (List<Map>) ctx.getData(Dict.MER_TRANS_LIST);
		if (merSubTransMaps != null && !merSubTransMaps.isEmpty()) {
			BigDecimal sumAmt = BigDecimal.ZERO; // 子交易金额相加
			List<SubTransData> onlineSubTransList = new ArrayList<SubTransData>();
			boolean isExistHoldPay=false;
			boolean isExistNoHoldPay=false;
			MathContext mc = new MathContext(5, RoundingMode.HALF_UP);
			BigDecimal subDeductCount = BigDecimal.ZERO;
			for (Iterator iterator = merSubTransMaps.iterator(); iterator
				...
				if(InteralFlag.YES.equals(inputData.getInteralflag())){
				    if(iterator.hasNext()){
				        BigDecimal subtransamt = StringUtil.parseBigDecimal(subMap.get(Dict.SUB_TRANS_AMT));
				        BigDecimal subdeductamt =  subtransamt.divide(inputData.getTransamt(), mc)
						    .multiply(inputData.getDeductamt())
							.setScale(2, RoundingMode.HALF_UP);
				        subDeductCount = subDeductCount.add(subdeductamt);
				        subMap.put(Dict.SUB_DEDUCT_AMT, subdeductamt);
				    }else{
				        subMap.put(Dict.SUB_DEDUCT_AMT,inputData.getDeductamt().subtract(subDeductCount));
				    }
				}
				//计算子订单打折金额
				if(CodeTypCd.ALIPAY.equals(inputData.getCodetypcd())){
					if(inputData.getDiscountableamt().compareTo(BigDecimal.ZERO) > 0){
						if(iterator.hasNext()){
					        BigDecimal subtransamt = StringUtil.parseBigDecimal(subMap.get(Dict.SUB_TRANS_AMT));
					        BigDecimal subdiscounttamt =  subtransamt.divide(inputData.getTransamt(), mc)
							    .multiply(inputData.getDiscountableamt())
								.setScale(2, RoundingMode.HALF_UP);
					        subDeductCount = subDeductCount.add(subdiscounttamt);
					        subMap.put(Dict.DISCOUNTABLE_AMOUNT, subdiscounttamt);
					    }else{
					        subMap.put(Dict.DISCOUNTABLE_AMOUNT,inputData.getDiscountableamt().subtract(subDeductCount));
					    }
					}
				}
				SubTransData subtrans=this.makeSubTrans(subMap, inputData);
				if (PayModeCd.HOLD.equals(subtrans.getSubMerAcct().getPaymodecd())) {
					isExistHoldPay=true;
				}else{
					isExistNoHoldPay=true;
				}
				sumAmt = sumAmt.add(subtrans.getTransamt());
			...
			// 验证总交易金额和子交易汇总金额是否相等
			if (inputData.getTransamt().compareTo(sumAmt) != 0) {
				throw new PeException(DictErrors.TRANS_AMT_NOT_MATCH);
			}
			//尚不支持冻结和非冻结支付模式混合
			if(isExistHoldPay&&isExistNoHoldPay){
				throw new PeException(DictErrors.HOLD_AND_NON_HOLD_NOT_SUPPORT);
			}
...
//				throw new PeException(DictErrors.MER_ONLY_DEBIT,new Object[]{transData
//						.getMernbr()});
//			} else if (CardTypCd.CREDIT.equals(transData.getPayercardtypcd())
//					&&CardTypCd.DEBIT.equals(merAcct.getCardtypecd())) {
//				throw new PeException(DictErrors.MER_ONLY_CREDIT,new Object[]{transData
//						.getMernbr()});
//			}
			
			if (CardTypCd.DEBIT.equals(transData.getPayercardtypcd())
					&&CardTypCd.CREDIT.equals(subMerAcct.getCardtypecd())) {
				throw new PeException(DictErrors.MER_ONLY_DEBIT,new Object[]{subTrans
						.getMernbr()});
			} else if (CardTypCd.CREDIT.equals(transData.getPayercardtypcd())
					&&CardTypCd.DEBIT.equals(subMerAcct.getCardtypecd())) {
				throw new PeException(DictErrors.MER_ONLY_CREDIT,new Object[]{subTrans
						.getMernbr()});
			}
			...
			if(PayTypCd.OTHACTSCAN.equals(transData.getPaytypcd())
				...
				Merbaseinfo merchant = MerbaseinfoDAO.selectByPrimaryKey(subTrans
						.getMernbr());
				if (merchant == null) {
					throw new PeException(DictErrors.MERCHANT_NOT_EXIST);
				}
		...
		if(!PayTypCd.OTHCYBER.equals(transData.getPaytypcd())){
			...
			FeeOutput feeOutput=((FeeService)this.getFeeService()).calcMerFeeParam(feeInput,transData.getMerAcct(),subTrans.getSubMerAcct());
