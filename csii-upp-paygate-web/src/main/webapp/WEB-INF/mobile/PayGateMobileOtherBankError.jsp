...
<body onload="LoadJs()">
	<form name="form1" action="#" method="post">
		<pe:hidden fieldList="Plain,Signature,OrderId,MerchantName,TransAmt"
			skipNULL="true" />
		<div class="logo">
		...
		<div class="main">
			<div class="cuo">
				<img src="images/phone/cuo.png" /> <span class="text_w">
				<c:if
						test="${PayStatus == '0'}">订单等待支付</c:if> <c:if
						test="${PayStatus == '2'}">订单处理失败
							<c:if test="${RespCode != '0000000'}">, 错误信息：${RespMessage}</c:if>
						</c:if> <c:if
						test="${PayStatus == '3'}">订单处理中</c:if> <c:if
						test="${PayStatus == '4'}">订单已经取消</c:if>
				</span>
			</div>
